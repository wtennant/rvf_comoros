# plot_compare_trans_reduce.R: Compare a summary statistic for
# all transmission reduction scenarios.
...
for (i in seq(1, length(scenarios)))
{
  ...
  sim <- group_by(sim, SAMPLE_ID, ISLAND_ID) %>%
    filter(TIME <= 528) %>% # Corresponds to 2015-07-01.
...
scen_free <- filter(sum_scenario, SCENARIO_ID == 1) %>%
  group_by(ISLAND_ID) %>%
  summarise(SCEN_FREE_INF = median(INF))

# Join onto the scenario summary data frame. 
sum_scenario <- left_join(sum_scenario, scen_free, by="ISLAND_ID")
...
gg_scenario <- ggplot(sum_scenario, aes(x=SCENARIO_ID, y=INF)) +
  ...
  scale_x_discrete("Reduction of within-island transmission (%)", labels=names(scenarios)) +
...
file_name <- "scenario_vec_control"
...
sum_sum_scenario <- sum_scenario %>%
  ...
  summarise(Q_500 = round(quantile(100*REL_INF/SCEN_FREE_INF, prob=0.5, na.rm=TRUE), digits=1),
            Q_975 = round(quantile(100*REL_INF/SCEN_FREE_INF, prob=0.975, na.rm=TRUE), digits=1),
            Q_025 = round(quantile(100*REL_INF/SCEN_FREE_INF, prob=0.025, na.rm=TRUE), digits=1))
sum_inf_scenario <- sum_scenario %>%
  group_by(SCENARIO_ID, SAMPLE_ID) %>%
  summarise(INF = sum(INF)) %>%
  summarise(Q_500 = round(quantile(INF, prob=0.5, na.rm=TRUE), digits=0),
            Q_975 = round(quantile(INF, prob=0.975, na.rm=TRUE), digits=0),
            Q_025 = round(quantile(INF, prob=0.025, na.rm=TRUE), digits=0))
sum_isl_scenario <- sum_scenario %>%
  group_by(SCENARIO_ID, ISLAND_ID) %>%
  summarise(Q_500 = round(quantile(INF, prob=0.5, na.rm=TRUE), digits=0),
            Q_975 = round(quantile(INF, prob=0.975, na.rm=TRUE), digits=0),
            Q_025 = round(quantile(INF, prob=0.025, na.rm=TRUE), digits=0))
