...
Simulation::Simulation(Parameters* pars, int chain_id)
{
    ...
    {
        S[t] = new double*[n_steps + 1];
        E[t] = new double*[n_steps + 1];
        I[t] = new double*[n_steps + 1];
        R[t] = new double*[n_steps + 1];
    ...
    file.open(static_cast<std::string>(RVF_ODIR) + "simulation_" +
        std::to_string(chain_id) + ".csv", std::fstream::out);
    ...
    else
    {
        ...
        file << "TIME,ISLAND_ID,AGE,S,E,I,R,ITER";
    }
    ...
    for (int t = start_time; t < end_time; ++t)
    {
        ...
        for (int i = 0; i < n_island; ++i)
        {
            ...
            for (int age = 0; age < n_age; ++age)
            {
                ...
                total_infectious[i] += I[t + 1][i][age];
                N[i] += S[t + 1][i][age] + E[t + 1][i][age] + I[t + 1][i][age] + R[t + 1][i][age];
