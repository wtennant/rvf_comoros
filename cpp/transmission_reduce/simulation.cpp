...
Simulation::Simulation(Parameters* pars)
{
    ...
    else
    {
        ...
        file << "SAMPLE_ID,TIME,ISLAND_ID,AGE,S,E,I,R,H";
    }
}
        ...
        for (int i = 0; i < n_island; ++i)
        {
            ...
            switch(RVF_TRANSMISSION)
            {
                case 0:     // Constant transmission.
                    foi = (1.0 - pars->vec_control[i])*pars->trans_scale[i];
                case 1:     // Linear transmission model.
                    foi = (1.0 - pars->vec_control[i])*(pars->ndvi_rate[i]*(data->ndvi.ndvi[i][t] - min_ndvi) + pars->trans_scale[i]);
                case 2:     // Exponential transmission model.
                    foi = (1.0 - pars->vec_control[i])*(exp(pars->ndvi_rate[i]*(data->ndvi.ndvi[i][t] - min_ndvi) + pars->trans_scale[i]));
            }
            ...
            for (int age = 0; age < n_age - 1; ++age)
            {
                ...
                // Copy all I into a pre-movement infectious vector H.
                H[t + 1][i][age] = I[t + 1][i][age];
            }
        }
